...
- 累计测试数、待查看/累计/已处理意见数；
- 支持今日、近 7 日、近 30 日切换的每日趋势和七种人格结果分布；
...
1. 首次安装时，在原 `huashao2-personality` Supabase 项目执行 `dashboard.sql`；已有看板执行 `pagination.sql` 和 `feedback-pagination.sql` 启用测试与意见分页。
